...
public class LoginPhaseListener implements PhaseListener {
    ...
    public void afterPhase(PhaseEvent event) {
        ...
        boolean paginaLogin2 = viewId.lastIndexOf("Usuario") > -1;
        
        if (existeUsuarioLogado() && paginaLogin) {
            nh.handleNavigation(facesContext, null, "/Principal?faces-redirect=true");
        } else if (!existeUsuarioLogado() && !paginaLogin) {
            nh.handleNavigation(facesContext, null, "/login?faces-redirect=true");
        }else if (!existeUsuarioLogado() && !paginaLogin2) {
            nh.handleNavigation(facesContext, null, "/login?faces-redirect=true");
        }   
    }
